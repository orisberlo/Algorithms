...
public final class StdRandom {
    ...
    /**
     * Returns a random integer uniformly in [0, n) which is not {@code i}.
     * 
     * @param n number of possible integers
     * @param i number in [0,n) 
     * @return a random integer uniformly between 0 (inclusive) and {@code n} (exclusive)
     * @throws IllegalArgumentException if {@code n <= 0}
     */
    public static int puncturedUniform(int n , int i) {
        if (n <= 0) throw new IllegalArgumentException("argument n must be positive");
        if (i < 0 || i>=n) throw new IllegalArgumentException("argument i must be within [0,n)");
    ...
    public static int[] randomDerangement(int n){
        if (n <= 0) throw new IllegalArgumentException("argument must be positive");
        if (n == 1) throw new IllegalArgumentException("No derangements of size 1");

    	int[] derangement = new int[n];
    	for(int i=0 ;i<n ; ++i){
    		derangement[i] = puncturedUniform(n,i);
    	}
    	return derangement;
    }
